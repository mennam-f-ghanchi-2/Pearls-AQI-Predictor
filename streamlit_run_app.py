...
<div class="hero">
    ...
    <h1>AQI PREDICTOR</h1>
    <div class="hero-sub">KARACHI, PAKISTAN &nbsp;|&nbsp; MULTI-STATION AGGREGATE</div>
</div>
""", unsafe_allow_html=True)
...
with st.spinner("Fetching live aggregated AQI data..."):
...
if raw:
    ...
    with col1:
        ...
        <div class="aqi-hero-card">
            <div class="aqi-label"><span class="live-dot"></span>LIVE AQI (CITY AVG)</div>
    ...
    with col3:
        ...
        <div class="metric-card" style="margin-bottom:12px">
            ...
            <div class="metric-value">{pm25}<span class="metric-unit">Index</span></div>
        </div>
        """, unsafe_allow_html=True)
    ...
    fig_gauge = go.Figure(go.Indicator(
        ...
        value=aqi,
        domain={'x': [0, 1], 'y': [0, 1]},
        title={'text': "City Average Index", 'font': {'color': '#8888aa', 'size': 14, 'family': 'Space Mono'}},
        ...
        gauge={
            ...
            'threshold': {'line': {'color': aqi_color, 'width': 4}, 'thickness': 0.75, 'value': aqi}
        }
    ))
    ...
    for col, (range_str, color, label, desc) in zip(cols, scale_data):
        with col:
            st.markdown(f"""
            <div style="background: rgba(255,255,255,0.03); border: 1px solid {color}33; border-top: 3px solid {color}; border-radius: 12px; padding: 1rem; text-align: center;">
                <div style="font-family: Space Mono; font-size: 1rem; font-weight: 700; color: {color}">{range_str}</div>
                <div style="font-size: 0.75rem; font-weight: 600; color: {color}; margin: 4px 0">{label}</div>
...
st.markdown("""
<div style="text-align: center; margin-top: 4rem; padding: 2rem 0; border-top: 1px solid #2a2a3a;">
